...
Presentation follow-up: use a shared core with Classic and Extra presentation profiles (working names). Verified appearance-preserving cache improvements apply to both. New non-cache optimizations, decorative simplifications and existing added guidance/display features belong to Extra. Shared correctness and portability fixes remain in both. PERF-19 provides the measurements; PROF-1–6 in [performance-plan.md](performance-plan.md#profile-separation-plan) define the inventory, configuration, implementation and acceptance work. The shipping default remains undecided until comparison. This does not authorize changing game rules or removing functional order/patience information.
