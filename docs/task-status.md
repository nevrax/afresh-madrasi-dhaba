...
PROF-1–6: Profile separation is planned in English in [performance-plan.md](performance-plan.md#profile-separation-plan), not implemented. Classic and Extra share game logic, correctness/portability fixes and verified appearance-preserving cache improvements. Extra contains new non-cache optimizations and existing added guidance/display features, including bowl hints/selection and flip/pickup cursors. A fixed-star background is Extra because freezing animation changes presentation; caching alone does not make that treatment common. The default profile is undecided pending visual and performance comparison. Next action: inventory additions (PROF-1), then introduce the typed profile configuration (PROF-2). No runtime, release or default changes are part of this planning update.
...
PERF-17 implementation now follows PROF-1–6: common cache improvements, then cheaper large griddle effects and simpler order decoration within Extra. Do not treat removing all small animations as sufficient; the combined decor cut still reaches only 39.86 FPS on Pi A. Original game implementation remains delivered, but full-clarity performance on basic hardware and cold stalls remain unresolved. The attribution pass changes test tooling and documentation only; no experimental cuts ship, and no distribution rebuild is needed. Both isolated Pi browsers are closed and the temporary display wake is restored. No system packages or GPU settings changed.
