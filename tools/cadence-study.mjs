...
const label=`${live?(reuse?"native-live-reuse":"native-live"):"cadence"}-${adapter}-${uncapped?'uncapped':'normal'}${process.argv.includes('--repeat')?'-repeat':''}${calibrated?'-calibrated':''}`,output=path.join(root,'.local-setup/logs',label);await mkdir(output,{recursive:true});
