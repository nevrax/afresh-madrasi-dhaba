...
const fields=['config','contextAttributes','canvas','geometry','results','work','fps','seconds','frames','playingFrames','raf','draw','warm','warmCpu','renderWork','unchangedFrames','dirtyScene','terminalPresentationGap','hiddenSamples','unfocusedSamples','state','simulationIncludingCommandsAndAudio','snapshot','missingAssets'];
...
await writeFile(path.join(root,'tests/reference/performance-cadence.json'),JSON.stringify({units:'Callback rates are not physical scanout FPS. Process CPU uses one core = 100%.',baselineRevision:'7d90d22',sourceHashScope:'Final integrated source; baseline and prototype runs precede integration. Native-production cases use the integrated renderer.',intelDisplayQuery:{width:2560,height:1440,reportedHz:240,observedCallbackRateDifferent:true},pi,intel,pixels,sourceHashes:hashes},null,2)+'\n');
