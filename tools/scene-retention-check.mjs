...
const root=path.resolve(import.meta.dirname,'..'), output=path.join(root,`.local-setup/logs/${layers?'scene-layers-check':'scene-retention-check'}`);await mkdir(output,{recursive:true});
...
try{
 ...
 const result=await page.evaluate(async layers=>{
  ...
  renderer.setPresentation('classic');layerExperiment?.dispose();return{checks,releasedBytes:renderer.sceneCacheBytes};
 },layers);
